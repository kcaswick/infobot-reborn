...
async def test_initialize_schema_creates_tables(db_conn: DatabaseConnection):
    """Test that initialize_schema creates required tables."""
    await initialize_schema(db_conn)
...
async def test_initialize_schema_sets_version(db_conn: DatabaseConnection):
...
async def test_initialize_schema_idempotent(db_conn: DatabaseConnection):
    """Test that initialize_schema can be called multiple times."""
    await initialize_schema(db_conn)
    await initialize_schema(db_conn)

    # Should still have only one version entry
    cursor = await db_conn.execute("SELECT COUNT(*) FROM schema_version")
    row = await cursor.fetchone()
    assert row[0] == 1


async def test_factoids_table_structure(db_conn: DatabaseConnection):
    """Test that factoids table has correct structure."""
    await initialize_schema(db_conn)

    # Get table info
    cursor = await db_conn.execute("PRAGMA table_info(factoids)")
...
async def test_factoids_table_constraints(db_conn: DatabaseConnection):
...
async def test_factoids_table_primary_key(db_conn: DatabaseConnection):
...
async def test_factoids_index_created(db_conn: DatabaseConnection):
...
async def test_reset_schema_drops_and_recreates(db_conn: DatabaseConnection):
    ...
    # Verify data exists
    cursor = await db_conn.execute("SELECT COUNT(*) FROM factoids")
    row = await cursor.fetchone()
    assert row[0] == 1

    # Reset schema
    await reset_schema(db_conn)
